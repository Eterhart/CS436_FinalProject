...
export function generatePassphrase(config: PassphraseConfig): string {
  ...
  for (let i = 0; i < config.wordCount; i++) {
    ...
    if (config.useFullWords) {
      word = word;
    } else {
      const truncateLength = Math.max(3, Math.floor(word.length * 0.6));
      word = word.substring(0, truncateLength);
    }

    if (config.capitalizeFirstLetter && i === 0) {
  ...
  if (config.includeNumber) {
    ...
    passphrase += config.separator + randomNumber;
  }
